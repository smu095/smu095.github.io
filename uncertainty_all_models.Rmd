...
* The model has **incorrectly classified** `r agg_df$n[1]` images. The mean uncertainty of the predicted class is `r mean(agg_df$mean_uncertainty[1])`.

* The model has **correctly classified** `r agg_df$n[2]` images. The mean estimated uncertainty is `r agg_df$mean_uncertainty[2]`.
...
As we saw in table 3.1.2, the mean $\tau_{jk}$ for the incorrectly classified images is `r agg_df$mean_ratio[1]` and `r agg_df$mean_ratio[2]` for the correctly classified images. Setting the referral threshold to the mean $\tau_{jk}$ for the incorrect classifications (`r agg_df$mean_ratio[1]`), we get:
...
Accuracy would rise to `r class2_df$n[2]/(sum(class2_df$n))`. This indicates that there may be some valuable information to be gathered from the runner-up predictions. So how do we determine which images to take a closer look at?

## Using the difference-uncertainty ratio
