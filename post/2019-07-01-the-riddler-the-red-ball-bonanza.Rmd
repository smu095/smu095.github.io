---
title: '#theriddler: The Red Ball Bonanza'
author: Sean
date: '2019-07-01'
...
This week's [Riddler Express](https://fivethirtyeight.com/features/whats-your-best-scrabble-string/) poses the following probability problem:
...
print_results(twohundred)
```

### Bonus: Varying the number of balls and buckets probabilities

Just for fun, let's plot two more functions:

1. $p(red)$ as a function of the number of balls $N$.
2. $p(red)$ as a function of the the probability $\alpha$ of selecting a bucket.

```python
def get_probs(n=100, alpha=0.5, **kwargs):
    """Return probability of drawing a red ball when selecting bucket at random."""
    x1 = y1 = range(1, n + 1)
    balls = it.product(x1, y1)

    results = {
        (red, blue): p(red, blue, urn_prob=alpha, **kwargs) 
        for red, blue in balls 
        if (red + blue) != 2*n
    }
...
```python
# Fixing bucket probability alpha = 0.5 and varying N
max_probs = []
N = [i for i in range(2, 501)]

# Calculating probabilities
for n in N:
    max_probs.append(get_probs(n=n, total_balls=2*n, red_balls=n))
    
# Plotting
plt.figure(figsize=(10, 6))

with plt.style.context("fivethirtyeight"):
    plt.plot(N, max_probs)
    plt.xlabel("$N$")
    plt.ylabel("$p(red)$")
    plt.title("Probability of drawing a red ball")
    plt.show()
```

![](/post/2019-07-01-the-riddler-the-red-ball-bonanza_files/N-urn.png)

```python
# Fixing number of coloured balls N = 100 and varying alpha
max_probs = []
N = 100
alpha = np.arange(0, 1.01, step=0.01)

# Calculating probabilities
for a in alpha:
    max_probs.append(get_probs(alpha=a, total_balls=2*N, red_balls=N))
    
# Plotting
plt.figure(figsize=(10, 6))

with plt.style.context("fivethirtyeight"):
    plt.plot(alpha, max_probs)
    plt.xlabel(r"$\alpha$")
    plt.ylabel("$p(red)$")
    plt.title("Probability of drawing a red ball")
    plt.show()
...
![](/post/2019-07-01-the-riddler-the-red-ball-bonanza_files/alpha-urn.png)
